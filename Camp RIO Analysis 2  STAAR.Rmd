...
A) total Averages (STAAR, 5th, 8th, 9th)
B) ONLY 5th, 9th grades (STAAR)
```{r}
#bin1, all grades
staar_N1 = 35 + 79 + 325 
...
both_ave_bin1 <- (4724.486*35  + 3775.738*325)/both_N1
both_ave_bin1
...
both_ave_bin0 <- (4108.176*17 + 3797.367*210)/both_N2
both_ave_bin0
...
#5th, 8th, 9th GRADEs (bin0), 0-2 visits
...
#E) 2 GRADEs 5, 9th, (bin1), >2 visits                NOT SIG DIFFERENT: ALL GRADES 5, 8, 9th ave. STAAR science scores (no difference)
#bio_sci_survey_20_21_bin1

STAAR_5_9_bin1<-bio_sci_survey_20_21_bin1%>%
  filter(grade_level != 8)
STAAR_5_9_bin1

STAAR_5_9_bin1$scale_score <- as.numeric(STAAR_5_9_bin1$scale_score)
STAAR_5_9_bin1

na.omit(STAAR_5_9_bin1)
STAAR_5_9_bin1
  
both_bin1 <- STAAR_5_9_bin1$scale_score
both_bin1

#2 GRADEs 5, 9th, (bin0), 0-2 visits
#bio_sci_survey_20_21 must filter for ("num_visits_bin == 0")
bio_sci_survey_20_21_bin0 <- bio_sci_survey_20_21 %>%
  filter(num_visits_bin == 0)
 

STAAR_5_9_bin0 <- bio_sci_survey_20_21_bin0%>%
  filter(grade_level != 8)

STAAR_5_9_bin0$scale_score <- as.numeric(STAAR_5_9_bin0$scale_score)
STAAR_5_9_bin0

na.omit(STAAR_5_9_bin0)
STAAR_5_9_bin0


both_bin0 <- STAAR_5_9_bin0$scale_score
both_bin0

#Whitney Mann U Test (non-parametric test since the 2 groups of data were not both normal)

wilcox.test(both_bin0, both_bin1)  #conclusion: Medians do NOT differ (they are not sig. different STAAR med. scores)

wilcox.test(both_bin0, both_bin1, alternative = "greater") #Not sig. different (no difference in STAAR median scores)
wilcox.test(both_bin0, both_bin1, alternative = "less") #Not sig. different (no diff. in STAAR median scores)
